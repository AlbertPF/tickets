<!-- Modal -->
<div id="modal-final-tickets" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="success-header-modalLabel">
...
<script>
    ...
    function finalizar_asignacion() {
        ...
        Swal.fire({
            ...
            preConfirm: () => {
                ...
                let formElement = document.getElementById("finalizartTcket");
                let formData = new FormData(formElement);

                return fetch('{{ route('final.ticketsAsig') }}', {
                    method: "POST",
                    body: formData,
                    headers: {
            ...
            if (result.isConfirmed) {
                Swal.fire({
                    ...
                    if (confirmar.isConfirmed || confirmar.dismiss === Swal.DismissReason.timer) {
                        $('#finalizartTcket')[0].reset();
                        $('#modal-final-tickets').modal('hide');
                        mostrar_tabla();
                        mostrar_tablaAsig();
                    }
                });
    ...
    function modalFinalizarTickets() {
        $("#alternative-page-datatable tbody").on('click', '.finalizar_ticket', function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#descripcion').val('');
                    $('#id_ticket').val(Asig.id_ticket);

                    $("#modal-final-tickets").modal('show');
                }
            });
