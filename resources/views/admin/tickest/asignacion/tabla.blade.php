<table id="selection-datatable" class="table table-striped nowrap w-100 tablaTicketsAsig">
    <thead>
        <tr>
            <th>N°</th>
            <th>Incidencia</th>
    ...
    <tbody>
        @foreach ($ticketsAsig as $ticketAsig => $row)
            <tr>
                <td scope ="row"><strong> {{ $ticketAsig + 1 }}</strong></td>
                <td>{{ $row->ticket->soporte->nombre ?? 'N/A' }}</td>
                <td>{{ $row->ticket->oficinaPersonal->personal->nombre . ' ' . $row->ticket->oficinaPersonal->personal->apellidoPaterno ?? 'N/A' }}</td>
                ...
                <td class="table-action">
                    <a href="javascript: void(0);" class="action-icon text-primary rounded ver_ticketAsig"
